...
my $sam = $ARGV[1];  # mapped results
my $output = $ARGV[2]; # length of sequence read
my $shift = $ARGV[3];  # base need to shift to the center of the read 
...
while (<SAM>) {
     ...
     my @mapping = split(/\t/);
     my $chrom = $mapping[2];
     my $pos = $mapping[3]+$shift;
     $genome{$chrom}{$pos} = $genome{$chrom}{$pos}+1;          
}
...
while (<REF>) {
  ...
  my $tran =  $transcript[1];  
     ...
     $knownGene{$name}{$tran}{"exonCount"} = $transcript[6];
     $knownGene{$name}{$tran}{"exonStarts"} = $transcript[7];
     $knownGene{$name}{$tran}{"exonEnds"} = $transcript[8];
}
...
foreach my $ID  (keys %isoGene )
{
    ...
    {
       print RRR "$ID\t$i_chrom\t$i_strand\t$i_start\t$i_end\t$size\t";
