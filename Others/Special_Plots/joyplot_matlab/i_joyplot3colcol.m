function i_joyplot3colcol(D,g,scalek,styleid,colortype)
...
% (c) Aug 2017
    ...
    if nargin<5, colortype=ones(size(D,1),1); end
    ...
    for kx=1:3
        switch styleid
            case 1
                hold on
                i_joy(D(g==kx-1,:),scalek,kx,colortype(g==kx-1));
            otherwise
                hold on
...
function i_joy(D,scalek,coln,colortype)
    ...
    for k=n:-1:1
       ...
       plot(1.1*p*(coln-1)+(1:p),...
            scalek*(k-1)*ones(1,p)+D(k,:),'color',cs);
