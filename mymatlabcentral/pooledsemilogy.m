function [pslope1]=pooledsemilogy(resA,resT,drawcloud)
...
%return;
    ...
    if drawcloud
      semilogy(resA,resT,'.','markersize',1);
      hold on
    end
	...
	semilogy(midx,pooledmean,'or','markersize',4,'MarkerFaceColor','w')
    if (1)
        %midx(end)=[];
        %pooledmean(end)=[];  
...
%        xlim([min(midx) max(midx)])
      ...
      semilogy([0 max(midx)],[pslope(2),max(midx)*pslope(1)+pslope(2)],'g');
      pslope1=pslope(1);
