function [nn,ns,ni,D]=perlegen_at_ensemblexon(geneid,transid,exonstart,exonend,logfile)
...
sql='SELECT p.rsid, p.chromosome, p.position, p.alleles, p.afr_freq, p.eur_freq, p.chn_freq, p.fst3, p.fst_ac, p.fst_ec, p.fst_ea';
...
if (nargout>3 & ~isempty(idx))
	...
	for (k=1:length(D.ancnuc)),
	      ...
	      if (ancx==mutx1)
		 D.daf(k)=1-D.afr_freq(k);
	      elseif (ancx==mutx2)
		 D.daf(k)=D.afr_freq(k);
	      else
		 D.daf(k)=-1;
	      end
	end
