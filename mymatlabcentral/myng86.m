function [dS,dN,dN_dS,VdS,VdN,St,Nt,nst,nat,nst2,nat2]=myng86(aln)
...
for j=i+1:n
		...
		[nst2(i,j), nat2(i,j)] = calculate_diff2(CodonSeq([i j],:),ns,na);
	...
	nst(j,i)=nst(i,j); nat(j,i)=nat(i,j);
        nst2(j,i)=nst2(i,j); nat2(j,i)=nat2(i,j);
...
function [nst, nat] = calculate_diff2(CodonSeq,ns,na)
	[n,m]=size(CodonSeq);
	if (n~=2) error ('must be two sequences!'); end
	nst=0;
	nat=0;
