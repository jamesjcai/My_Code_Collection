...
static int count_qfc, r, lim;  static BOOL ndtsrt, fail;
...
{ return x < -50.0 ? 0.0 : exp(x); }
   ...
   {
      extern int count_qfc,lim;
      count_qfc = count_qfc + 1;
      if ( count_qfc > lim ) longjmp(env,1);
   }
